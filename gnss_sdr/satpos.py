...
def satpos(transmitTime, prnList, eph, settings):
  ...
  for satNr in range(numOfSatellites):
    ...
    dtr = F * eph[prn].e * eph[prn].sqrtA * math.sin(E)
    #Calculate the true anomaly
    nu = math.atan2(math.sqrt(1-(eph[prn].e))**2*math.sin(E), math.cos(E) - eph[prn].e)
    #Compute angle phi
    phi = nu + eph[prn].omega
    #Constrain phi to between 0 and 2pi
    phi = phi % 2*gpsPi
    #Correct argument of latitude
    u = phi + eph[prn].C_uc * math.cos(2*phi) + eph[prn].C_us * math.sin(2*phi)
    #Correct radius
    r = a * (1 - eph[prn].e*math.cos(E)) + eph[prn].C_rc*math.cos(2*phi) + eph[prn].C_rs*math.sin(2*phi)
    #Correct inclination
    i = eph[prn].i_0 + eph[prn].iDot*tk + eph[prn].C_ic*math.cos(2*phi) + eph[prn].C_is*math.sin(2*phi)
    ...
    satPositions[satNr].append(math.cos(u)*r * math.cos(Omega) - math.sin(u)*r*math.cos(i)*math.sin(Omega))
    satPositions[satNr].append(math.cos(u)*r * math.sin(Omega) - math.sin(u)*r*math.cos(i)*math.cos(Omega))
    satPositions[satNr].append(math.sin(u)*r * math.sin(i))
